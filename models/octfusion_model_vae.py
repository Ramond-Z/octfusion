...
from models.networks.diffusion_networks.ldm_diffusion_util import set_requires_grad
...
class OctFusionModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.autoencoder = load_dualoctree(
            conf=vq_conf, ckpt=opt.vq_ckpt, opt=opt)
        ...
        if self.isTrain:
            ...
            self.optimizer = optim.AdamW(
                [p for p in self.autoencoder.parameters() if p.requires_grad],
                lr=opt.lr
            )
        ...
        if opt.ckpt is None and os.path.exists(
            os.path.join(
                opt.logs_dir,
                opt.name,
    ...
    def forward(self):
        self.autoencoder.train()
        model_out = self.autoencoder_module(
            self.octree_in,
            self.octree_gt,
            self.batch['pos']
        )
        ...
        output = loss_func(
            self.batch,
            model_out,
            self.vq_conf.loss.loss_type,
            kl_weight=self.vq_conf.loss.kl_weight
        )
    ...
    def inference(self, save_folder="results_vae"):
        self.autoencoder.eval()
        output = self.autoencoder.forward(
            octree_in=self.batch['octree_in'], evaluate=True)
        ...
        # output['neural_mpu']是一个函数。
        self.get_sdfs(output['neural_mpu'], self.batch_size, bbox)
        self.export_mesh(save_dir, index=0)

        # save the input point cloud
        pointcloud = trimesh.PointCloud(
            vertices=self.batch['points'][0].cpu().points.numpy())
    ...
    def export_mesh(self, save_dir, index=0, level=0, clean=False):
        ...
        for i in range(ngen):
            ...
            if vtx.size == 0 or faces.size == 0:
                ...
                return
            # [0,sz]->[bbmin,bbmax]  把vertex放缩到[bbmin, bbmax]之间
    ...
    def load_ckpt(self, ckpt, autoencoder, load_opt=False):
        ...
        if load_opt:
            ...
            print(colored('[*] training start from: %d' %
                  self.start_iter, 'green'))
            self.optimizer.load_state_dict(state_dict['opt'])
            print(
                colored('[*] optimizer successfully restored from: %s' % ckpt, 'blue'))
